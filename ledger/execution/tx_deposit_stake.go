...
func (exec *DepositStakeExecutor) sanityCheck(chainID string, view *st.StoreView, viewSel core.ViewSelector, transaction types.Tx) result.Result {
	...
	if (tx.Purpose == core.StakeForValidator || tx.Purpose == core.StakeForGuardian) && stake.SPAYWei.Cmp(types.Zero) != 0 {
		return result.Error("SPAY has to be zero for validator or guardian stake deposit!").
			WithErrorCode(result.CodeInvalidStake)
	}
